...
- `meta_distribution_report.py`
  - 功能：遍历 `data/processed/*/meta.csv`（默认 CA/GBA/GLA/SD 四个数据集）并判断每个字段是否为离散/连续变量，离散列输出频次表与柱状图，连续列输出直方图与描述性统计；同时写入 `summary.json` 方便后续分析。
  - 运行：`python utils/baselines/eda/meta_distribution_report.py --bins 40 --unique-threshold 30 --ratio-threshold 0.1`，如需指定其他 processed 目录可传 `--data-root`，图表与 CSV 默认写入 `results/meta_eda/<dataset>/{plots,tables}`。
  - 输出：每个数据集生成 `tables/*.csv`（频次或直方图分箱）、`plots/*.png`（离散柱状图或直方图）、`summary.json`；顶层生成 `results/meta_eda/meta_overview.json` 方便统一查看。
