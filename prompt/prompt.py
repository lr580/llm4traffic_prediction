from utils.datasets.handler import DatasetHanlder
import numpy as np
import datetime
...
class Prompt():
    ...
    def generate(self, handler:DatasetHanlder, X:np.ndarray, i:int, j:int):
...
class PromptPlain(Prompt):
    ...
    def generate(self, handler:DatasetHanlder, X:np.ndarray, i:int, j=-1):
        prompt = TASK_DESC
        startTime = handler.timeCalc.getStartTime(i)
        endTime = startTime + datetime.timedelta(hours=1)
        futureTime = endTime + datetime.timedelta(hours=1)
        week = handler.timeCalc.getWeek(i)
        prompt += f'探测器位于{handler.dataset.space}。'
